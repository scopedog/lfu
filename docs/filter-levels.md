...
because there is no name-index walk.

**Measured 2026-08-17**, and the cliff is not merely flat — for the attributes a
filter reads it is *absent*. `--name` did 100,015 linkea reads over a
100,112-object MDT for **1.6%**, where the same predicate costs 27% on ldiskfs.
And tier 2 could not be provoked at all: an object padded with a 65 KB foreign
xattr pushed *the pad* into the xattr directory while `trusted.lma`, `lov`,
`link` and `som` all stayed in a 424-byte SA nvlist, because Lustre moves the
