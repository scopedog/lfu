...
sudo timeout 300 build/lfind-kmdt /dev/lfu_scan 2>/dev/null |
...
sudo insmod src/kernel/lfu_ring.ko dev=testfs-MDT0000-osd
sudo timeout 300 build/lfind-kmdt --size +1G /dev/lfu_scan 2>/dev/null | sed 's/^/    /'
sudo rmmod lfu_ring 2>/dev/null || true

echo
echo "=== the same inode read straight off the device with debugfs ==="
INO=$(sudo build/lfind-ldiskfs --size +1G /lustre/mdt.img 2>/dev/null | grep -oE 'ino=[0-9]+' | cut -d= -f2)
echo "    inode = $INO"
