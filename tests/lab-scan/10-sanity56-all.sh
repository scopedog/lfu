#!/bin/bash
# Stage 10 (was 7): sanity.sh 56* -- the lfs find tests -- before and after, on the
...
echo "=== AFTER: base + all seven patches"
cd $L && git am /home/nishida/0*.patch > /dev/null && git log --oneline -3
run_one after
